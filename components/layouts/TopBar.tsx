...
export function TopBar() {
  const { currentPersona, switchPersona, logout } = useAuth();
  ...
  return (
    <header className="h-14 border-b bg-white flex items-center justify-between px-6 flex-shrink-0">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-gray-900">{currentPersona.name}</p>
          <p className="text-[11px] text-gray-500">{currentPersona.title} — {currentPersona.organization}</p>
        </div>
      </div>
