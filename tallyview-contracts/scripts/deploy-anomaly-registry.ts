// Deploy AnomalyRegistry behind a UUPS proxy, linked to AuditLedger, and grant
// SYSTEM_ROLE to the relay service.
// Usage: npx hardhat run scripts/deploy-anomaly-registry.ts --network tallyview-testnet
...
async function main() {
  ...
  console.log("Deploying AnomalyRegistry with account:", deployer.address);
  ...
  const anomalyRegistry = await upgrades.deployProxy(
    AnomalyRegistryFactory,
    [auditLedgerAddress],
    { initializer: "initialize", kind: "uups" },
  );
  await anomalyRegistry.waitForDeployment();

  const proxyAddress = await anomalyRegistry.getAddress();
  const implAddress = await upgrades.erc1967.getImplementationAddress(proxyAddress);

  const SYSTEM_ROLE = ethers.keccak256(ethers.toUtf8Bytes("SYSTEM_ROLE"));
  const tx = await anomalyRegistry.grantRole(SYSTEM_ROLE, relayAddress);
  await tx.wait();

  console.log("-------------------------------------------");
  console.log("AnomalyRegistry deployed successfully");
